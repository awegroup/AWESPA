...
# Repo-specific paths used by the 100 kW case-study setup.
...
RESULTS_DIR = PROJECT_ROOT / "case_studies" / "case_1_100kw_sizing"
...
def make_system_file(
    ...
    # Repo-specific awesIO component keys from the 100 kW template.
    ...
    for kite in KITE_OPTIONS:
        for tether in TETHER_OPTIONS:
            ...
            for generator_kw in GENERATOR_POWERS_KW:
                rows.append(
                    make_system_file(template_system, kite, force_kn, generator_kw)
                )
...
def extract_power_curve_rows(
    ...
    for curve in power_data["power_curves"]:
        ...
        for wind_speed, power_w in zip(wind_speeds, cycle_power_w):
            rows.append(
                {
                    **{key: value for key, value in metadata.items() if key != "system_path"},
...
def tether_force_linestyle(force_kn: float) -> str:
    """Return a stable matplotlib line style for a tether-force level."""
